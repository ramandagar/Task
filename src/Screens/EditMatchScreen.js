...
import { updateSlot } from '../Slices/MatchSlice';
...
const EditMatchScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (index !== undefined) {
      const { date, startTime, endTime, days } = slots[index];
  ...
  const saveChanges = () => {
    ...
    const isOverlap = slots.some((slot, i) => {
      if (i !== index) {
    ...
    if (isOverlap) {
      ...
      dispatch(updateSlot({ index, slot: updatedSlot }));
      navigation.goBack();
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Edit Your Match Timings</Text>
      ...
      <Button mode='outlined' onPress={saveChanges}>Save chnages</Button>
    </View>
  );
};
